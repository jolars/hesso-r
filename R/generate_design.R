...
#' @param rho Correlation between predictors (corresponding to choice in
#'   `rho_type`)
#' @param s Number of signals (nonzero coefficients)
#' @param rho_type Type of correlation between predictors. `"constant"` gives
#'   uniform correlation between all predictors. `"auto"` means that predictors
#'   are "autocorrelated" with adjacent predictors by the amount specified in
#'   `rho`.
...
generate_design <- function(n,
  ...
  if (rho != 0) {
    if (rho_type == "auto") {
      inds <- 1:p
      covar <- rho^abs(outer(inds, inds, "-"))
      u <- chol(covar)

      x <- x %*% t(u)
      sigma <- sqrt((t(beta) %*% covar %*% beta) / snr)
    ...
    sigma <- sqrt((t(beta) %*% diag(p) %*% beta) / snr)
  }
